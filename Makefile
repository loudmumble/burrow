.PHONY: build build-local build-stager check-anvil build-anvil build-stager-evasion build-stager-packed build-all test smoke clean tidy sizes verify
...
STAGER    := stager
BUILD_DIR := build
VERSION   ?= $(shell git describe --tags --always 2>/dev/null || echo "v4.0.0")
...
# Minimal stager for IoT/embedded (ARM, MIPS)
build-stager-embedded:
	@mkdir -p $(BUILD_DIR)
	@for target in linux/arm linux/arm64 linux/mips linux/mipsle; do \
		GOOS=$${target%/*}; \
		GOARCH=$${target#*/}; \
		echo "  stager $$GOOS/$$GOARCH..."; \
		CGO_ENABLED=0 GOOS=$$GOOS GOARCH=$$GOARCH $(GO) build \
			-ldflags="-s -w" -trimpath \
			-o $(BUILD_DIR)/$(STAGER)-$$GOOS-$$GOARCH ./cmd/stager/ || exit 1; \
	done
	@echo "Embedded stagers built."
...
build-stager-evasion: build-anvil
	...
	@cp -n cmd/stager/memexec_*.go cmd/stager/_build/ 2>/dev/null || true
...
# Smoke tests: build binary, start server, test WebUI+API actually work
smoke: build-local
