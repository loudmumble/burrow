...
import (
	...
	"github.com/loudmumble/burrow/internal/plugin/stdlib"
...
// Mention stdlib to prevent unused import
var _ = stdlib.PortscanModule{}
...
func init() {
	...
	serverCmd.Flags().String("webui", "", "Enable WebUI dashboard (default: 0.0.0.0:9091)")
	serverCmd.Flags().Lookup("webui").NoOptDefVal = "0.0.0.0:9091"
...
func runServer(cmd *cobra.Command, _ []string) {
	...
	// Default: always enable management API on localhost if no other WebUI/API options are set.
	// This ensures local tools like 'topology' work out of the box.
	...
	if enableAPI || webuiEnabled || tuiEnabled {
		...
		// Generate a separate ECDSA cert for the web server — browsers don't
		// support Ed25519 TLS, so the agent transport cert can't be reused.
		var webTLS *tls.Config
		if tlsCfg != nil {
			webCert, webCertErr := certgen.GenerateSelfSignedECDSA("Burrow WebUI", 365*24*time.Hour)
			if webCertErr != nil {
				fmt.Fprintf(serverErrLog, "[!] Failed to generate WebUI TLS cert: %v\n", webCertErr)
				os.Exit(1)
			}
			webTLS = &tls.Config{
		...
		apiEnabled := enableAPI || webuiEnabled || tuiEnabled
		webSrv := web.NewServer(webAddr, mgr, events, apiToken, apiEnabled, webTLS, webuiEnabled)
...
func handleAgentConn(conn net.Conn, mgr *session.Manager, transportName string) {
	...
	info := &session.Info{
		...
		CreatedAt: time.Now(),
		Active:    true,
		NumCPU:    handshake.NumCPU,
		Debugged:  handshake.Debugged,
	}
	...
	// Auto-restore tunnels, routes, and TAP from previous session.
	if prev := mgr.PrevState(handshake.Hostname); prev != nil {
		go func() {
			time.Sleep(500 * time.Millisecond) // let data stream acceptor start
			restored := 0
			for _, tc := range prev.Tunnels {
				if _, err := mgr.AddTunnel(sessionID, tc.Direction, tc.ListenAddr, tc.RemoteAddr, tc.Protocol); err != nil {
					fmt.Fprintf(serverErrLog, "[!] Auto-restore tunnel %s %s→%s failed: %v\n", tc.Direction, tc.ListenAddr, tc.RemoteAddr, err)
				} else {
					fmt.Fprintf(serverLog, "[*] Tunnel %s %s→%s auto-restored\n", tc.Direction, tc.ListenAddr, tc.RemoteAddr)
					restored++
				}
			}
			for _, cidr := range prev.Routes {
			...
			if prev.TapAddr != "" {
				if err := mgr.StartTap(sessionID, prev.TapAddr); err != nil {
					fmt.Fprintf(serverErrLog, "[!] TAP auto-restore failed for %s: %v\n", sessionID, err)
				} else {
					fmt.Fprintf(serverLog, "[*] TAP auto-restored for reconnected agent %s\n", handshake.Hostname)
					restored++
				}
			}
			if restored > 0 {
				fmt.Fprintf(serverLog, "[*] Auto-restored %d items for reconnected agent %s\n", restored, handshake.Hostname)
			}
			mgr.ClearPrevState(handshake.Hostname)
		}()
	}
...
func serverCommandLoop(ctrl net.Conn, mgr *session.Manager, sessionID string) error {
	...
	for {
		...
		case msg := <-msgCh:
			...
			case protocol.MsgTapStartAck:
				ack, err := protocol.DecodeTapStartAck(msg)
				if err != nil {
					fmt.Fprintf(serverErrLog, "[!] Bad TAP start ack: %v\n", err)
					continue
				}
				if ack.Error != "" {
					fmt.Fprintf(serverErrLog, "[!] Agent TAP start failed: %s\n", ack.Error)
					mgr.HandleTapAck(sessionID, ack.Error)
				} else {
					fmt.Fprintf(serverLog, "[*] Agent TAP mode ready on %s\n", ack.Interface)
					mgr.HandleTapAck(sessionID, "")
				}
...
func acceptDataStreams(sess *mux.Session, mgr *session.Manager, sessionID string) {
	...
	for {
		...
		case 0x02: // Remote tunnel connection
			go handleRemoteTunnelStream(stream, mgr, sessionID)
		case 0x03: // TAP data stream
			mgr.HandleTapDataStream(sessionID, stream)
		default:
...
func handleRemoteTunnelStream(stream net.Conn, mgr *session.Manager, sessionID string) {
	defer stream.Close()
	defer func() {
		if r := recover(); r != nil {
	...
	conn, err := net.DialTimeout("tcp4", remoteAddr, 10*time.Second)
	if err != nil {
	...
	// Look up which tunnel ID owns this remote address for per-tunnel byte attribution.
	// Falls back to session-level-only tracking if no match found (e.g. race with tunnel removal).
	tunnelID := mgr.FindTunnelByRemoteAddr(sessionID, remoteAddr)

	// Bidirectional relay with per-tunnel and per-session byte tracking.
	done := make(chan struct{}, 2)
	go func() {
		// FROM TARGET TO AGENT = OUTBOUND (bytes leaving the server toward agent)
		defer func() { done <- struct{}{} }()
		var cw relay.CountingWriter
		if tunnelID != "" {
			cw = relay.CountingWriter{Target: stream, Added: func(n int64) { mgr.AddTunnelBytesOut(sessionID, tunnelID, n) }}
		} else {
			cw = relay.CountingWriter{Target: stream, Added: func(n int64) { mgr.AddBytesOut(sessionID, n) }}
		}
		relay.CopyBuffered(cw, conn)
		if tc, ok := stream.(*net.TCPConn); ok {
			tc.CloseWrite()
		}
	}()
	go func() {
		// FROM AGENT TO TARGET = INBOUND (bytes arriving from agent at the server)
		defer func() { done <- struct{}{} }()
		var cw relay.CountingWriter
		if tunnelID != "" {
			cw = relay.CountingWriter{Target: conn, Added: func(n int64) { mgr.AddTunnelBytesIn(sessionID, tunnelID, n) }}
		} else {
			cw = relay.CountingWriter{Target: conn, Added: func(n int64) { mgr.AddBytesIn(sessionID, n) }}
		}
		relay.CopyBuffered(cw, stream)
		if tc, ok := conn.(*net.TCPConn); ok {
			tc.CloseWrite()
		}
	}()
	<-done
	time.Sleep(50 * time.Millisecond) // drain: let second direction flush in-flight data
	conn.Close()
	stream.Close()
	<-done
}
