...
var tunnelCmd = &cobra.Command{
	Use:   "tunnel",
	Short: "TCP port forwarding and HTTP relay tunnels",
	Long: `Tunnel subcommands for port forwarding and HTTP-based relaying.

Port forwarding:
  tunnel local    — listen locally, forward to remote target
  tunnel remote   — listen on agent, forward to operator

HTTP relay (web-based pivoting):
  tunnel server   — HTTP relay server (runs on target)
  tunnel client   — HTTP relay client with local SOCKS5 (runs on attacker)
  Use -s/--secure for AES-256-GCM encrypted, WAF-evasive mode.`,
}
