...
func (m tuiModel) handleDetailKey(msg tea.KeyMsg) (tea.Model, tea.Cmd) {
	...
	// Shift+Y: copy tunnel/route data
	case "Y":
		if m.detailTab == tuiTabTunnels && m.cursor < len(m.tunnels) {
			t := m.tunnels[m.cursor]
...
func (m tuiModel) handleExecKey(msg tea.KeyMsg) (tea.Model, tea.Cmd) {
	...
	case "backspace":
		...
		if len(v) > 0 {
			runes := []rune(v)
			m.inputValues[0] = string(runes[:len(runes)-1])
		}
	default:
		for _, r := range key {
			if r >= 32 {
				m.inputValues[0] += string(r)
			}
		}
	}
	return m, nil
...
func (m tuiModel) handleDownloadKey(msg tea.KeyMsg) (tea.Model, tea.Cmd) {
	...
	default:
		// Accept typed chars and pasted text (multi-char strings)
		for _, r := range key {
			if r >= 32 {
				m.inputValues[0] += string(r)
			}
...
func (m tuiModel) handleUploadKey(msg tea.KeyMsg) (tea.Model, tea.Cmd) {
	...
	case "backspace":
		...
		if len(v) > 0 {
			runes := []rune(v)
			m.inputValues[m.inputCursor] = string(runes[:len(runes)-1])
		}
	default:
		for _, r := range key {
			if r >= 32 {
				m.inputValues[m.inputCursor] += string(r)
			}
		}
	}
	return m, nil
...
func (m tuiModel) viewDetail(b *strings.Builder) {
	...
	b.WriteString("\n")
	b.WriteString(renderHelpBar([]string{"↑/k up", "↓/j down", "esc back", "^T TUN", "s SOCKS5", "Y copy", "F fp", "K kill", "o output", "P profile", "? help"}) + "\n")
