...
const (
	...
	// MsgTapStart is sent by the proxy to request TAP (L2) mode activation.
	MsgTapStart MessageType = 0x53
	// MsgTapStartAck is sent by the agent to confirm TAP mode is active.
	MsgTapStartAck MessageType = 0x54
	// MsgTapStop is sent by the proxy to deactivate TAP mode.
	MsgTapStop MessageType = 0x55
	...
	// MsgSleep tells the agent to disconnect and sleep for a duration.
	MsgSleep MessageType = 0x80
	// MsgUpgrade pushes a new binary to the agent/stager for in-memory execution.
	MsgUpgrade MessageType = 0x81
	// MsgUpgradeAck confirms or rejects the upgrade.
	MsgUpgradeAck MessageType = 0x82
...
// SleepPayload tells the agent to disconnect and sleep for a duration.
type SleepPayload struct {
	Seconds int64 `json:"seconds"`
}

// EncodeSleep creates a MsgSleep message.
func EncodeSleep(seconds int64) (*Message, error) {
	data, err := json.Marshal(&SleepPayload{Seconds: seconds})
	if err != nil {
		return nil, fmt.Errorf("marshal sleep: %w", err)
	}
	return &Message{Type: MsgSleep, Payload: data}, nil
}

// DecodeSleep extracts sleep duration from a MsgSleep message.
func DecodeSleep(msg *Message) (*SleepPayload, error) {
	if msg.Type != MsgSleep {
		return nil, fmt.Errorf("%w: got %s, want Sleep", ErrTypeMismatch, msg.Type)
	}
	var p SleepPayload
	if err := json.Unmarshal(msg.Payload, &p); err != nil {
...
// UpgradePayload pushes a new binary to the agent for in-memory execution.
type UpgradePayload struct {
	Binary []byte   `json:"binary"` // ELF/PE binary
	Args   []string `json:"args"`   // CLI arguments for the new binary
}

// EncodeUpgrade creates a MsgUpgrade message.
func EncodeUpgrade(binary []byte, args []string) (*Message, error) {
	data, err := json.Marshal(&UpgradePayload{Binary: binary, Args: args})
	if err != nil {
		return nil, fmt.Errorf("marshal upgrade: %w", err)
	}
	return &Message{Type: MsgUpgrade, Payload: data}, nil
}

// DecodeUpgrade extracts the upgrade payload.
func DecodeUpgrade(msg *Message) (*UpgradePayload, error) {
	if msg.Type != MsgUpgrade {
		return nil, fmt.Errorf("%w: got %s, want Upgrade", ErrTypeMismatch, msg.Type)
	}
	var p UpgradePayload
	if err := json.Unmarshal(msg.Payload, &p); err != nil {
		return nil, fmt.Errorf("unmarshal upgrade: %w", err)
	}
	return &p, nil
}

// UpgradeAckPayload confirms or rejects the upgrade.
type UpgradeAckPayload struct {
	Error string `json:"error,omitempty"`
}

// EncodeUpgradeAck creates a MsgUpgradeAck message.
func EncodeUpgradeAck(errStr string) (*Message, error) {
	data, err := json.Marshal(&UpgradeAckPayload{Error: errStr})
	if err != nil {
		return nil, fmt.Errorf("marshal upgrade ack: %w", err)
	}
	return &Message{Type: MsgUpgradeAck, Payload: data}, nil
}

// DecodeUpgradeAck extracts the upgrade ack.
func DecodeUpgradeAck(msg *Message) (*UpgradeAckPayload, error) {
	if msg.Type != MsgUpgradeAck {
		return nil, fmt.Errorf("%w: got %s, want UpgradeAck", ErrTypeMismatch, msg.Type)
	}
	var p UpgradeAckPayload
	if err := json.Unmarshal(msg.Payload, &p); err != nil {
		return nil, fmt.Errorf("unmarshal upgrade ack: %w", err)
	}
	return &p, nil
}
...
// --- TAP (Layer 2) payload helpers ---

// TapStartPayload tells the agent which interface to capture on.
type TapStartPayload struct {
	Interface string `json:"interface,omitempty"` // CIDR to claim on agent (e.g. "10.10.10.200/24")
}

// TapStartAckPayload is sent by the agent to confirm or reject TAP mode.
type TapStartAckPayload struct {
	Error     string `json:"error,omitempty"`
	Interface string `json:"interface,omitempty"` // actual interface used
}

// EncodeTapStart creates a TapStart message.
func EncodeTapStart(p *TapStartPayload) (*Message, error) {
	data, err := json.Marshal(p)
	if err != nil {
		return nil, fmt.Errorf("marshal tap start: %w", err)
	}
	return &Message{Type: MsgTapStart, Payload: data}, nil
}

// DecodeTapStart extracts a TapStartPayload from a message.
func DecodeTapStart(msg *Message) (*TapStartPayload, error) {
	if msg.Type != MsgTapStart {
		return nil, fmt.Errorf("%w: got %s, want TapStart", ErrTypeMismatch, msg.Type)
	}
	var p TapStartPayload
	if len(msg.Payload) > 0 {
		if err := json.Unmarshal(msg.Payload, &p); err != nil {
			return nil, fmt.Errorf("unmarshal tap start: %w", err)
		}
	}
	return &p, nil
}

// EncodeTapStartAck creates a TapStartAck message.
func EncodeTapStartAck(p *TapStartAckPayload) (*Message, error) {
	data, err := json.Marshal(p)
	if err != nil {
		return nil, fmt.Errorf("marshal tap start ack: %w", err)
	}
	return &Message{Type: MsgTapStartAck, Payload: data}, nil
}

// DecodeTapStartAck extracts a TapStartAckPayload from a message.
func DecodeTapStartAck(msg *Message) (*TapStartAckPayload, error) {
	if msg.Type != MsgTapStartAck {
		return nil, fmt.Errorf("%w: got %s, want TapStartAck", ErrTypeMismatch, msg.Type)
	}
	var p TapStartAckPayload
	if err := json.Unmarshal(msg.Payload, &p); err != nil {
		return nil, fmt.Errorf("unmarshal tap start ack: %w", err)
	}
	return &p, nil
}

// EncodeTapStop creates a TapStop message (empty payload).
func EncodeTapStop() *Message {
	return &Message{Type: MsgTapStop}
}
