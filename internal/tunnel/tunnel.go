...
func (t *Tunnel) StartWithContext(ctx context.Context) error {
	network := transport.NetworkForAddr(t.ListenAddr, "tcp")
	ln, err := net.Listen(network, t.ListenAddr)
	if err != nil {
...
func (t *Tunnel) handleConn(ctx context.Context, local net.Conn) {
	...
	network := transport.NetworkForAddr(t.RemoteAddr, "tcp")
	remote, err := dialer.DialContext(ctx, network, t.RemoteAddr)
	if err != nil {
...
func (rt *ReverseTunnel) StartWithContext(ctx context.Context) error {
	...
	network := transport.NetworkForAddr(rt.config.AgentAddr, "tcp")
	ln, err := net.Listen(network, rt.config.AgentAddr)
	if err != nil {
		cancel()
...
func (rt *ReverseTunnel) handleIncoming(ctx context.Context, incoming net.Conn) {
	...
	network := transport.NetworkForAddr(rt.config.LocalTarget, "tcp")
	target, err := net.DialTimeout(network, rt.config.LocalTarget, 10*time.Second)
	if err != nil {
...
func (rt *ReverseTunnel) ConnectOutbound(ctx context.Context) error {
	for attempt := 0; attempt <= rt.config.MaxRetries; attempt++ {
		...
		network := transport.NetworkForAddr(rt.config.AgentAddr, "tcp")
		conn, err := net.DialTimeout(network, rt.config.AgentAddr, 10*time.Second)
		if err != nil {
...
func relayConns(a, b net.Conn, bytesAB, bytesBA *atomic.Int64) {
	...
	cp := func(dst, src net.Conn, counter *atomic.Int64) {
		...
		cw := relay.CountingWriter{Target: dst, Added: func(n int64) {
			if counter != nil {
				counter.Add(n)
			}
		}}
		relay.CopyBuffered(cw, src)
