...
// udpBufPool pools 64KB buffers for UDP relay goroutines to reduce GC pressure.
...
type Opts struct {
	...
	// DebugRelay enables per-read/write logging on TCP relays.
	DebugRelay bool

	// EnableIPv6 enables IPv6 and ICMPv6 support. Default: false (mandate IPv4).
	EnableIPv6 bool
}
...
func New(opts Opts) (*Stack, error) {
	...
	if opts.EnableIPv6 {
		...
		transProtos = append(transProtos, icmp.NewProtocol6)
	}
	...
	// Route all traffic through our NIC.
	routes := []tcpip.Route{
	...
	// Disable TCP SYN cookies — SYN cookies prevent proper connection tracking
	// which breaks scan tools (nmap) that rely on seeing SYN-ACK responses.
...
func (s *Stack) InjectPacket(raw []byte) {
	...
	case 6:
		// Silently drop IPv6 if not enabled to prevent protocol confusion/leaks.
		if s.enableIPv6 {
...
// canLog returns true if at least 200ms have passed since the last connection
// log. Used to rate-limit per-connection logging during scan floods while
// preserving visibility during normal operation.
func (s *Stack) canLog() bool {
	s.logMu.Lock()
	defer s.logMu.Unlock()
	now := time.Now()
	if now.Sub(s.lastLog) < 200*time.Millisecond {
		return false
	}
	s.lastLog = now
	return true
...
func (s *Stack) handleTCP(req *tcp.ForwarderRequest) {
	...
	req.Complete(false)
	// Tune gvisor endpoint — relay.TuneConn only handles *net.TCPConn,
	// this is a gonet.TCPConn so we set options directly on the endpoint.
	ep.SocketOptions().SetDelayOption(false)  // disable Nagle
	ep.SocketOptions().SetKeepAlive(true)     // enable keepalive

	netstackConn := gonet.NewTCPConn(&wq, ep)

	dialer := net.Dialer{Timeout: tcpDialTimeout}
	network := transport.NetworkForAddr(dst, "tcp")
	// If the stack is IPv4-only, force the dialer to use tcp4.
	if !s.enableIPv6 {
		network = "tcp4"
	}
	remote, err := dialer.DialContext(s.ctx, network, dst)
	if err != nil {
		netstackConn.Close()
		if s.debugRelay {
	...
	go func() {
		defer s.wg.Done()
		relayTCP(netstackConn, remote, s.debugRelay, s.logger)
	}()
}
...
// debugConn wraps a net.Conn with per-read/write logging (only used when DebugRelay is true).
...
func (d *debugConn) Read(p []byte) (int, error) {
	n, err := d.Conn.Read(p)
	if n > 0 || err != nil {
		d.logger.Printf("relay-debug [%s] Read %d bytes, err=%v", d.label, n, err)
	}
	return n, err
}
...
func (d *debugConn) Write(p []byte) (int, error) {
	...
	if n > 0 || err != nil {
		d.logger.Printf("relay-debug [%s] Write %d bytes, err=%v", d.label, n, err)
	}
	return n, err
}

func (d *debugConn) Close() error {
	d.logger.Printf("relay-debug [%s] Close", d.label)
	return d.Conn.Close()
}

// relayTCP copies data bidirectionally between two connections. Both connections
// are closed when relay returns. If debug is true, wraps connections with logging.
func relayTCP(a, b net.Conn, debug bool, logger *log.Logger) {
	var ra, rb net.Conn = a, b
	if debug {
		ra = &debugConn{Conn: a, label: "netstack", logger: logger}
		rb = &debugConn{Conn: b, label: "target", logger: logger}
	}

	done := make(chan struct{}, 2)
	go func() {
		defer func() { done <- struct{}{} }()
		relay.CopyBuffered(rb, ra)
		if tc, ok := rb.(interface{ CloseWrite() error }); ok {
			tc.CloseWrite()
		}
	}()
	go func() {
		defer func() { done <- struct{}{} }()
		relay.CopyBuffered(ra, rb)
		if tc, ok := ra.(interface{ CloseWrite() error }); ok {
			tc.CloseWrite()
		}
	}()
	<-done
